...
/* Private functions ---------------------------------------------------------*/
#pragma message "MAX_STATIC_ALLOC_SIZE: " STR(MAX_STATIC_ALLOC_SIZE)
...
void uart_print_request(USBD_HandleTypeDef *pdev)
{
    if(pdev->request.bRequest == 0x00)
        uart_send_str("GET_STATUS", 1);
    if(pdev->request.bRequest == 0x01)
        uart_send_str("CLEAR_FEATURE", 1);
    if(pdev->request.bRequest == 0x03)
        uart_send_str("SET_FEATURE", 1);
    if(pdev->request.bRequest == 0x05)
        uart_send_str("SET_ADDRESS", 1);
    if(pdev->request.bRequest == 0x06)
        uart_send_str("GET_DESCRIPTOR", 1);
    if(pdev->request.bRequest == 0x07)
        uart_send_str("SET_DESCRIPTOR", 1);
    if(pdev->request.bRequest == 0x08)
        uart_send_str("GET_CONFIGURATION", 1);
    if(pdev->request.bRequest == 0x09)
        uart_send_str("SET_CONFIGURATION", 1);
    uart_send_str(" ", 0);
    uart_send_hex(pdev->request.bmRequest, 1);
    uart_send_str(" ", 0);
    uart_send_hex(pdev->request.wValue, 2);
    uart_send_str(" ", 0);
